...
private:
    ...
    std::vector<std::vector<float> > A;// coefficients of all the variables

    std::vector<float> B;  //constraints

    std::vector<float> C;  //target function coefficients
...
public:
    ...
    //find the greatest coefficient of constraints in the objective function
    int findPivotColumn() {
    ...
    void dual_problem(){
        ...
        while (!end) {
            ...
            bool result = simplexAlgorithmCalculataion(); // edit to dual
            if (result) {
...
int main() {
    ...
    //
    //    float C[] = {-6, -5, -4, 0, 0, 0};  //should initialise the c arry here
    //    float B[] = {180, 300, 240};  // should initialise the b array here
    ...
    float C[]= {7,4,4,0,0,0};  //should initialise the c array here
    float B[]={3,3,5};  // should initialise the b array here
    float a[3][6] = {    //should intialise the A[][] array here
            { 4,  1,  1, 1,0,0},
            { 1,  2,  0,0,1,0},
            { 0,  0.5 ,3,0,0,1}
